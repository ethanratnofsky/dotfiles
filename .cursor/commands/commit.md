...
You must investigate multiple sources **in priority order** to discover how this project structures its commit messages. Use the **first source that provides clear guidance**; combine sources if one fills gaps left by another.
...
Keep this platform context for later steps — it affects where contribution templates live, how issue/PR references are formatted, and which footer syntax is idiomatic.
...
1. Write the final message to a temporary file to preserve formatting:
   ```
   echo '<message>' > /tmp/cursor_commit_msg.txt
...
- **Never push.** This command only creates local commits.
...
- **Never fabricate file names or changes.** Only reference files and diffs that actually exist in the output of the Git commands you ran.
